        ...
        for col in range(self.size):
            ...
            if target_piece and target_piece.piece_type == GRAY:
                if col < x:
                    ...
                    while target_col >= x and self.grid[y][target_col - 2] is not None:
                        target_col -= 1
        ...
        for row in range(self.size):
            ...
            if target_piece and target_piece.piece_type == GRAY:
                if row < y:  # Gray piece is above the purple piece
                    ...
                    while target_row > y and self.grid[target_row - 2][x] is not None:
                        target_row -= 1
    ...
    def get_piece_positions(self):
        ...
        for y in range(self.size):
            for x in range(self.size):
                piece = self.grid[y][x]
                if piece:
                    positions[(x, y)] = piece.piece_type
        return positions
...
def ucs_solve(board):
    priority_queue = []
    initial_state = tuple(sorted(board.get_piece_positions().items()))
    heappush(priority_queue, (0, initial_state, [], board))

    visited = set()
    visited.add(initial_state)

    while priority_queue:
        current_cost, current_state, path, current_board = heappop(priority_queue)

        if current_board.is_solved():
            return path

        for pos, piece_type in current_board.get_piece_positions().items():
            for direction in DIRECTIONS:
                new_board = current_board.clone()
                piece = new_board.grid[pos[1]][pos[0]]
                ...
                if new_board.move_piece(piece, direction):
                    if piece.piece_type == PURPLE:
                        new_board.handle_repulsion(*piece.position)
                    elif piece.piece_type == RED:
                        new_board.handle_attraction(*piece.position)

                    new_state = tuple(sorted(new_board.get_piece_positions().items()))

                    if new_state not in visited:
                        visited.add(new_state)
                        heappush(priority_queue,
                                 (current_cost + 1, new_state, path + [(piece_type, direction)], new_board))
...
def main():
    ...
    board.add_piece(Piece(GRAY, (2, 1), fixed=True))
    board.add_piece(Piece(PURPLE, (0, 2)))
    board.add_piece(Piece(RED, (1, 2)))
    board.voids = [(1, 0), (2, 2), (3, 3)]
    ...
    ucs_solution = ucs_solve(board)
    if ucs_solution:
        print("\nUCS Solution found:")
        for move in ucs_solution:
            print(f"Move {move[0]} piece {move[1]}")
    else:
